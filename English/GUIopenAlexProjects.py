...
def searchAuthorIds(firstName, lastName):
    authorSearch = requests.get("https://api.openalex.org/authors?search=" + firstName + "%20" + lastName)
    results = json.loads(authorSearch.text)
    authors = results["results"]

    openAlexIds = []
    for author in authors:
        if re.match(unidecode.unidecode("^" + firstName + ".*" + lastName), unidecode.unidecode(author["display_name"]), re.IGNORECASE):
...
def assembleProject():
    ...
    #absoluteCurrentDirectory = os.path.abspath("../")
    directory = absoluteCurrentDirectory + "//projects//"
    ...
    projectTitle = projectEntry.get().replace("\n", " ")
    projectTitle = re.sub(r'[^a-zA-Z0-9 ]+', '-', projectTitle)

    projectSummary = summaryText.get(1.0, tkinter.END).replace("\n", "")
    file = open(directory + projectTitle + ".txt", "w", encoding="utf8")
    ...
    summaryText.delete(1.0, tkinter.END)
    ...
    for id in authorIds:
        ...
        for page in range(pages):
            query = requests.get("https://api.openalex.org/works?filter=author.id:" + id + "&page=" + str(page+1) + "&per-page=200")  # 200 per page
            # For more pages: https://api.openalex.org/works?page=2&per-page=200
            ...
            for work in worksById:
                ...
                cleanTitle = str(cleanTitle).replace("\n", "").replace("||", "--")
                date = work["publication_year"]
                type = work["type"]
                line = cleanTitle + "||" + str(date) + "||" + str(type) + "||" + str(authors)
                # Remove duplicates here
                flag = True
                for content in virtualResume:
                    if cleanTitle in content:
                        print(cleanTitle)
                        flag = False
                        break
                if flag:
    ...
    file.writelines(projectTitle + "\n")
    file.writelines(projectSummary + "\n")

    for line in virtualResume:
        file.writelines(line + "\n")
    messagebox.showinfo("Project Ready", "Project:  " + projectTitle + " - ready.")
    file.close()
...
def loadTreeView():
    ...
    for author in authorList:
        treeSelectName.insert("", tk.END, text=author["display_name"], values=(str(author["id"])[21:], author["works_count"]))
...
def on_rightClick_treeSelectName(event):
    ...
    query = requests.get("https://api.openalex.org/works?filter=author.id:" + id + "&per-page=20")  # 20 per page
    results = json.loads(query.text)
    worksById = results["results"]
...
window = tk.Tk()
window.title("Search OpenAlex Authors")
# window.state('zoomed')

firstName_label = tk.Label(window, text="Author's First Name:")
firstName_label.grid(row=0, column=0)
firstNameEntry = tk.Entry(window)
firstNameEntry.grid(row=0, column=1)

lastName_label = tk.Label(window, text="Author's Last Name:")
...
choices_label = tk.Label(window, text="Authors to add to the project:")
choices_label.grid(row=3, column=4)
...
treeGenerateResume.grid(row=4, column=4, columnspan=2)

empty_label = tk.Label(window, text="    ")
empty_label.grid(row=6, column=2)

project_label = tk.Label(window, text="Project Title:")
project_label.grid(row=7, column=0, sticky="E")
projectEntry = tk.Entry(window, width=100)
projectEntry.grid(row=7, column=1, columnspan=4, sticky="W")

summary_label = tk.Label(window, text="Summary:")
summary_label.grid(row=8, column=0, sticky="E")
summaryText = tk.Text(window, width=100, height=10)
summaryText.grid(row=8, column=1, columnspan=4, sticky="W")

generateProjectButton = tk.Button(window, text="Generate Project", command=assembleProject)
generateProjectButton.grid(row=9, column=1)
